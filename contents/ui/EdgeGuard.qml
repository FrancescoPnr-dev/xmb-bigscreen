...
Item {
    ...
    function disableSystemEdges() {
        if (active) return
        active = true
        run("b=$(kreadconfig6 --file kwinrc --group XmbBigScreen --key SavedElectricBorders); " +
            "if [ -z \"$b\" ]; then " +
              "v=$(kreadconfig6 --file kwinrc --group Windows --key ElectricBorders); " +
              "kwriteconfig6 --file kwinrc --group XmbBigScreen --key SavedElectricBorders \"${v:-unset}\"; " +
            "fi; " +
    ...
    function restoreSystemEdges() {
        ...
        run("b=$(kreadconfig6 --file kwinrc --group XmbBigScreen --key SavedElectricBorders); " +
            "if [ -n \"$b\" ]; then " +
              ...
              "fi; " +
              "kwriteconfig6 --file kwinrc --group XmbBigScreen --key SavedElectricBorders --delete; " +
              "qdbus6 org.kde.KWin /KWin reconfigure; " +
            "fi")
    }
